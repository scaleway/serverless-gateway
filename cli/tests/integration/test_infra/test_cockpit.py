...
def scw_project() -> Generator[account.Project, None, None]:
    """Return the project."""
...
def test_ensure_cockpit_activated(scw_project: account.Project):
    client = Client.from_config_file_and_env()
...
def test_get_metrics_push_url(scw_project: account.Project):
    client = Client.from_config_file_and_env()
    client.default_project_id = scw_project.id
    api = sdk.CockpitV1Beta1API(client)
